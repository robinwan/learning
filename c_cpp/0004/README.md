...
"NEEDED"表示依赖的库，注意"NEEDED"并不是libopensource.so.xxx.0这样的完整名字，而是libopensource.so.xxx（该名字是SONAME，我们在编译libopensource共享库时通过"-Wl,-soname"指定，[参考文献 2](http://bottomupcs.sourceforge.net/csbu/x4012.htm)中对SONAME有介绍）。
